...
const loginToBackend = async (userInfo, login, navigate, setError) => {
  try {
    const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/login`, userInfo, {
...
function LoginPageInner() {
    ...
    if (hash.includes('access_token')) {
      ...
      if (accessToken) {
        axios
          ...
          .catch((err) => {
            console.error('❌ 리디렉션 후 사용자 정보 오류:', err);
            setError('구글 로그인 실패');
          });
      }
    }
  ...
  return (
    <div className="min-h-screen bg-white px-4 flex flex-col items-center justify-center relative">
      <button onClick={() => navigate(-1)} className="absolute top-4 left-4 text-blue-400 text-2xl">
        <FaArrowLeftLong />
      </button>
      ...
      <div className="space-y-4 w-full max-w-xs">
        <button onClick={googleLogin} className="flex items-center justify-center gap-2 w-full py-2 border border-gray-300 rounded bg-white hover:bg-gray-50 shadow">
          <img src="/googleimg.png" alt="Google" className="w-5 h-5" />
          <span className="text-sm text-gray-700">구글로 시작하기</span>
        </button>

        <button onClick={kakaoLogin} className="flex items-center justify-center gap-2 w-full py-2 rounded bg-[#FEE500] hover:brightness-95 shadow">
        ...
        <div className="text-center mt-4">
          ...
          <button onClick={() => navigate('/register')} className="text-sm text-blue-400 font-semibold hover:underline mt-1">
            회원가입 하러가기
          </button>
        </div>
      </div>
